...
import rospy
from detection.msg import bbox
...
if os.name == "nt":
    ...
    lib = CDLL("/home/srikar/darknet/libdarknet.so", RTLD_GLOBAL)
...
def main(argv):
    ...
    thresh = 0.25
    darknet_path="/home/srikar/darknet/"
    ...
    distance_msg=[]
    msg=bbox()
    pub = rospy.Publisher('detections', bbox)
    key = ''
        ...
        if err == sl.ERROR_CODE.SUCCESS:
           ...
           # print(len(detections))
            ...
            for detection in detections:
                ...
                y_extent_msg.append(y_extent)
                x_extent_msg.append(x_extent)
                # Coordinates are around the center
                x_coord = int(bounds[0] - bounds[2]/2)
                y_coord = int(bounds[1] - bounds[3]/2)
                x_coord_msg.append(x_coord)
                y_coord_msg.append(y_coord)
            ...
            #cv2.imwrite("/home/srikar/res.jpg",image)
            key = cv2.waitKey(5)

            log.info("FPS: {}".format(1.0 / (time.time() - start_time)))
            msg.x_coord=x_coord_msg
            msg.y_coord=y_coord_msg
            msg.x_extent=x_extent_msg
            msg.y_extent=y_extent_msg
            msg.distance=distance_msg
            msg.num=len(distance_msg)
            pub.publish(msg)
            x_coord_msg = []
            y_coord_msg = []
            x_extent_msg= []
